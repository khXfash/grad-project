...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _sendReport() async {
    ...
    try {
      ...
      final subject =
          'BraceletSync Weekly Report - ${DateFormat('MMM d, yyyy').format(DateTime.now())}';
  ...
  String _buildReportBody({
    ...
    sb.writeln('BraceletSync – Weekly Health Report');
    ...
    if (readings.isEmpty) {
      ...
      final avgScore =
          readings.map((r) => r.stressScore).reduce((a, b) => a + b) /
              readings.length;
      final maxScore =
          readings.map((r) => r.stressScore).reduce(math.max);
      final minScore =
          readings.map((r) => r.stressScore).reduce(math.min);
      final avgHR =
          readings.map((r) => r.heartRate).reduce((a, b) => a + b) /
              readings.length;

      sb.writeln('Average Stress Score : ${avgScore.toStringAsFixed(1)}/100');
      sb.writeln('Max Stress Score     : $maxScore/100');
      sb.writeln('Min Stress Score     : $minScore/100');
      sb.writeln('Avg Heart Rate       : ${avgHR.toStringAsFixed(0)} bpm');
      sb.writeln('');
      sb.writeln('── Detailed Readings ──');
      for (final r in readings) {
        sb.writeln(
            '${fmtFull.format(r.timestamp)}  Stress: ${r.stressScore}/100  HR: ${r.heartRate} bpm  Temp: ${r.skinTemp.toStringAsFixed(1)}°C');
      }
    }
    ...
    if (moods.isEmpty) {
      ...
      for (final m in moods) {
        ...
        sb.writeln(
            '${fmtFull.format(m.timestamp)}  ${m.emotion[0].toUpperCase()}${m.emotion.substring(1)} $src  Stress@time: ${m.stressScore}/100$note');
      }
    }

    sb.writeln('');
    sb.writeln('───────────────────────────────────────');
    sb.writeln('This report was automatically generated by BraceletSync.');
    return sb.toString();
  }
